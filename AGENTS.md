...
### 约束 4 — 任务拆解分工（2026-08-02 定稿）

**Hermes 拆「方向与边界」，codex 拆「实现路径」**——Hermes 不替 codex 拆到原子：

- 单域编码任务（加功能 / 修 bug / 小重构）→ **粗粒度派发**：目标 + 边界 + 验收标准 + 「任务较大可自行拆解执行」，授权 codex 自拆
- 跨域混合（编码 + 调研 + 协调其他工具）→ Hermes 拆完再派
- 需要并行的多任务 → delegate_task 并行派发
- 有硬约束（别动某文件 / 限时 / 风格要求）→ Hermes 在派发层写清，不靠 codex 猜
